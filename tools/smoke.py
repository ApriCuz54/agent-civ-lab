...
async def smoke_one(router, key):
    ...
    for kind, system, prompt, k in probes():
        try:
            ...
            lat.append(r.secs)
            ok = parse.parse_move(r.text) if kind == "move" else parse.parse_price(r.text)
    ...
    row.update(valid_rate=round(valid / n, 2), errors=len(errs), first_error=(errs[0] if errs else ""),
...
def main():
    ...
    if a.models and os.path.exists(path):   # merge with earlier rows
        ...
        rows = old + rows
    write_csv(path, rows)
    print_table(rows, ["key", "provider", "valid_rate", "errors", "served_mismatches", "calls_per_min", "g0_pass", "first_error"])
